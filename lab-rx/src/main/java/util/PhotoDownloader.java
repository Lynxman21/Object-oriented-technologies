...
public class PhotoDownloader {
    ...
    public Observable<Photo> searchForPhotos(List<String> searchQuery) throws IOException, InterruptedException {
        List<Observable<Photo>> obs = new ArrayList<>();
        for (String querry:  searchQuery) {
            if (querry.equals("") || querry == null) {
                continue;
            }

            Observable<Photo> singleObs = searchForPhotos(querry).subscribeOn(Schedulers.io());
            obs.add(singleObs);
        }
