...
public class Student {
    ...
    public static Optional<Student> findByIndexNumber(final int indexNumber) {
        ...
        try (ResultSet r = QueryExecutor.read(sql,args)) {
            if (r.next()) return Student.findById(r.getInt("id"));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return Optional.empty();
    ...
    public Map<Course, Float> createReport() {
        ...
        Map<Course, Integer> gradesCount = new HashMap<>();

        String sql = "SELECT g.grade, c.id, c.name FROM grade g " +
                "JOIN course c ON c.id = g.course_id " +
                "WHERE student_id = ?;";
        Object[] args = {id};
        try {
            ResultSet res =  QueryExecutor.read(sql,args);

            while (res.next()) {
                Course c =  new Course(res.getInt("id"),res.getString("name"));
                if (!report.keySet().contains(c)) {
                    report.put(c, res.getFloat("grade"));
                    gradesCount.put(c,1);
                }
                else {
                    Float val = report.get(c);
                    report.put(c, (val * gradesCount.get(c) + res.getFloat("grade"))/(gradesCount.get(c) + 1));
                    gradesCount.put(c,gradesCount.get(c) + 1);
                }
            }
            return report;
        } catch (SQLException e) {
